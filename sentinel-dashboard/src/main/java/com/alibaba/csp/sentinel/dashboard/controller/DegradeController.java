...
//            List<DegradeRuleEntity> rules = sentinelApiClient.fetchDegradeRuleOfMachine(app, ip, port);
            ...
            List<DegradeRuleEntity> filterRules = rules.stream().filter(s -> s.getIp().equals(ip) && s.getPort().equals(port)).collect(Collectors.toList());
    ...
    /**
     * 发布规则时根据应用名ip和端口来发布，但是会更新到nacos配置中
     *
     * warning 集群服务只会应用于修改的一台
     *
     * @param app 应用名称
     * @param ip 应用ip
     * @param port 应用端口
